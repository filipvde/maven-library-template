...
- [ ] add workflow for when develop branch merges to master branch.
    - 1) input field needed for pom.xml version in master branch without -SNAPSHOT
    - 2) input field needed for pom.xml version in develop branch with -SNAPSHOT
    - 3) add optional boolean field to choose to automatically create a tag
    - 4) add optional boolean field to choose to automatically create a release => in this case, a tag is automatically created
    - 5) merge flow:
        - A) change pom.xml version in pom.xml of develop to version without -SNAPSHOT
        - B) merge develop to master
        - C) do the actions based on the input fields
        - D) merge master into develop
